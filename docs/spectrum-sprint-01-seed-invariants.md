...
### I8 — the frequency line's own service survives
Where a cell's first line is `<range> <trailing text>`, the trailing text (minus footnote tokens) must appear among that row's service names.
**Status: FAILS.** 327 violations, all in `spectrum_region_allocations` (0 of 552 AU rows). Finding #16, cleared by Sprint 5. Baseline: `tests/fixtures/seed_dropped_service_baseline.json`.

Added after the spec was first written, when the data turned up the defect. The JS implementation was validated against an independent Python one: the suite passes against a baseline generated by the Python version, so the two agree row for row.
...
- **I4 and I8 overlap in intent.** Both are heuristics for "the extractor mangled this cell"; Sprint 5 should clear them together.
