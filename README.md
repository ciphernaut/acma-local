...
The server speaks two transports: **stdio** (Claude Desktop, LM Studio local) and **Streamable HTTP/SSE** on `:3000` (LM Studio 0.3.17+, networked MCP hosts). Both modes share the same 19-tool catalog.
...
- **Geospatial export** — site/device results carry coordinates and can be rendered as a map layer: `export_geojson` for QGIS and web maps, `export_kml` for Google Earth. See [`docs/geospatial-export.md`](docs/geospatial-export.md) for the shared column conventions and how to choose.
