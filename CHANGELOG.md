...
### Added
- **`export_qml`** generates a QGIS layer style for the same cached result: point symbol, labels, and a **map tip** listing every attribute. The map tip is the piece that makes a layer feel usable — QGIS shows nothing on click by default, so exported attributes appear "missing" until one exists (and `View > Show Map Tips` is on, which is a QGIS-wide toggle no file can set). Save it beside the data with the same basename and QGIS applies it on load. Works over GeoJSON or KML, since ExtendedData keeps the field names. Tested for well-formed XML, escaping, expression-safe field names and tip/column agreement — but not for how it renders, as there is no QGIS in CI.
