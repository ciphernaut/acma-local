...
`src/db.ts` declares `TABLE_METADATA` — DDL + post-load indexes for every materialised table. There are 31 tables + `meta` + the FTS5 virtual table `applic_text_block_fts`:
